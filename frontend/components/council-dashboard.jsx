...
function SuburbPanel({ id, onDrillIn, onBrief }) {
      ...
      <div>
        ...
        <div className="insight">
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
            <button className="btn btn-ink" onClick={onBrief}>Open brief <CIcon.arrow/></button>
            <button className="btn" onClick={onDrillIn}>View suburb detail</button>
          </div>
        </div>
      </div>
    </aside>
...
// ─── Hero artboard ──────────────────────────────────────────────────────
function CouncilHero({ onDrillIn, onBrief, onExport, onFilters }) {
  const [sel, setSel] = React.useState('carlton');
  const [range, setRange] = React.useState('Fortnight');
  const [activeAge, setActiveAge] = React.useState({ '65-74': true, '75-84': true, '85+': true, 'under': false });
  const [activeNeeds, setActiveNeeds] = React.useState({ Social: true, Food: true, Transport: true, Wellbeing: true, Health: false, Financial: false, Safety: false });
  const [serviceSignals, setServiceSignals] = React.useState({ free: true, accessible: true, after5: false, phone: false });
  const Check = ({ on }) => on
    ? <svg width="10" height="10" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M2 6.5L5 9.5L10 3.5"/></svg>
    : null;
  return (
      ...
      <div className="topbar">
        ...
        <div className="right">
          <div className="seg" role="tablist" aria-label="Time range">
            {['7 days', 'Fortnight', '90 days', 'This year'].map((r) => (
              <button key={r} className={range === r ? 'active' : ''} onClick={() => setRange(r)}>{r}</button>
            ))}
          </div>
          <button className="btn" onClick={onFilters}><CIcon.filter/> Filters</button>
          <button className="btn btn-ink" onClick={onExport}><CIcon.export/> Export brief</button>
        </div>
      </div>
      ...
      <div className="main">
        <div className="rail">
          <div className="filter">
            <h4>Age band</h4>
            {[['65-74', '65 – 74'], ['75-84', '75 – 84'], ['85+', '85+'], ['under', 'Under 65']].map(([k, label]) => (
          ...
          <div className="filter">
            ...
            <div className="pill-list">
              {Object.keys(activeNeeds).map((n) => (
                <button key={n} className={`pill ${activeNeeds[n] ? 'on' : ''}`} onClick={() => setActiveNeeds((p) => ({ ...p, [n]: !p[n] }))}>{n}</button>
              ))}
            </div>
          </div>
          ...
          <div className="filter">
            ...
            {[['free', 'Free or low-cost'], ['accessible', 'Wheelchair access'], ['after5', 'Open after 5pm'], ['phone', 'Phone bookings']].map(([k, label]) => (
...
function CouncilDetail({ onBack, onExport }) {
    ...
    <div className="council detail">
      <div className="topbar">
        <div style={{ display: 'flex', alignItems: 'center', gap: 22 }}>
          ...
          <div className="crumbs">
            <button className="btn" style={{ padding: '4px 10px', fontSize: 12 }} onClick={onBack}>← Back to map</button>
            <span style={{ color: 'var(--ink)' }}>Carlton, 3053</span>
          </div>
        </div>
        <div className="right">
          <button className="btn">Compare with…</button>
          <button className="btn btn-ink" onClick={onExport}><CIcon.export/> Generate area brief</button>
        </div>
      </div>
...
function Modal({ open, title, sub, children, onClose, footer }) {
  React.useEffect(() => {
    if (!open) return;
...
// ─── Brief modal (suggested actions for the selected suburb) ───────────
  ...
  return (
    <Modal
      ...
      footer={<>
        ...
        <button className="btn btn-ink"><CIcon.export/> Send to outreach team</button>
      </>}
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <ol style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 12 }}>
          {[
            ...
            <li key={i} style={{ display: 'grid', gridTemplateColumns: '24px 1fr auto', gap: 12, alignItems: 'start' }}>
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--ink-4)', marginTop: 2 }}>0{i + 1}</span>
              <div>
...
function ExportModal({ open, onClose }) {
  ...
  const [includeMood, setIncludeMood] = React.useState(true);
  const [includePhrases, setIncludePhrases] = React.useState(false);
  return (
    <Modal
      open={open}
      ...
      footer={<>
        <button className="btn" onClick={onClose}>Cancel</button>
        <button className="btn btn-ink" onClick={onClose}><CIcon.export/> Generate {fmt.toUpperCase()}</button>
      </>}
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        <div>
          ...
          <div className="seg" style={{ display: 'inline-flex' }}>
            {[['pdf', 'PDF brief'], ['csv', 'CSV (raw)'], ['link', 'Share link']].map(([k, l]) => (
              <button key={k} className={fmt === k ? 'active' : ''} onClick={() => setFmt(k)}>{l}</button>
            ))}
          </div>
        </div>
        <div>
            ...
            {[
              ...
              <label key={k} className="opt" style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, cursor: 'pointer' }} onClick={() => fn(!v)}>
                <span className={`check ${v ? 'on' : ''}`}>
                  {v && <svg width="10" height="10" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M2 6.5L5 9.5L10 3.5"/></svg>}
                </span>
                {l}
              </label>
            ))}
  ...
  return (
    <Modal
      ...
      footer={<>
        ...
        <button className="btn btn-ink" onClick={onClose}>Apply</button>
      </>}
    >
...
// ─── CouncilApp — top-level state wiring hero ↔ drill + modals ─────────
function CouncilApp() {
...
Object.assign(window, { CouncilHero, CouncilDetail, CouncilApp });
